...
class TestNormalizeSamples(unittest.TestCase):
    ...
    def test_bizinfo_sample(self):  # 라이브 jsonArray shape 매핑
        ...
        self.assertEqual(r.source_id, "PBLN_000000000123787")
        self.assertEqual(r.key, "bizinfo:PBLN_000000000123787")
        self.assertEqual(r.deadline, "2026-07-14")  # reqstBeginEndDe 종료일
        self.assertEqual(r.specialized_agency, "청주시문화산업진흥재단")
        self.assertTrue(r.title and r.url and r.agency)
...
class TestBizinfoLive(unittest.TestCase):  # (c) 라이브 OpenAPI 전환: 파싱·매핑·key 안정성
    ...
    def test_key_stable_vs_static(self):  # 라이브 pblancId를 구식 URL에 넣어도 동일 key 파생(고아화 방지)
        ...
        # 라이브 source_id는 pblancId 그대로. 구 정적 스냅샷 URL은 ?pblancId=<동일ID> 형태였으므로
        # 그 URL에서 _pblanc_id를 역추출하면 라이브 source_id와 일치 → 전환 후 DB key 불변.
        old_style_url = f"https://www.bizinfo.go.kr/web/lay1/bbs/S1T122C128/AS/74/view.do?pblancId={live.source_id}"
        self.assertEqual(_pblanc_id(old_style_url), live.source_id)
        self.assertEqual(live.key, "bizinfo:PBLN_000000000123787")
    ...
    def test_adapter_parses_jsonArray(self):  # 응답 루트 jsonArray 추출 + 빈 페이지 break
        ...
        self.assertEqual(raws[0].raw["pblancId"], "PBLN_000000000123787")
